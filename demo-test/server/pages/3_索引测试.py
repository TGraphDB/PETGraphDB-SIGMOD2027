...
st.set_page_config(page_title="索引测试", page_icon="📈", layout="wide")
...
readWriteTests = ['append', 'update', 'etpc', 'aggmax']  # 'oltp', 'olap', , 'r100'
dbList = ['TGKI', 'TGS']  # 'MA', 'TGC', 'TGK', 'TGKI', 'TGL', 'TGLI', 'N1', 'N2' 'TGK',
...
def fetchLatestJID(tdb, p__p):
    pp = [0]
    sql = '''SELECT "ID" jid, created_at ct, duration, reqcnt req_cnt FROM "Jenkins-Test" jt WHERE jt.device='data' and 
        db=%s and dataset=%s and "mTpSize"=%s and test=%s and "maxCon"=%s
        ORDER BY jid DESC LIMIT 1'''
    def hhhh(row):
        resultArr = tdb.execute(sql, (row['db'].lower(), row['dataset'], row['tp_size'][1:], row['test'], row['max_con']))
        ...
        if len(resultArr) > 0:
            ...
            tmp = resultArr[0]
            return ','.join(xxx), tmp['ct'], tmp['duration'], tmp['req_cnt']
        else:
...
FROM verified_case WHERE jenkins_id_trial IN %s GROUP BY jenkins_id_trial ORDER BY jenkins_id_trial DESC''', (idArr,))
        p__p.progress(pp[0] % 100, text=f":relieved: loading {row['db']}@{row['dataset']}{row['tp_size']}^{row['max_con']}")
        pp[0] += 2
        if len(resultArr) > 0:
...
def moss550w():
    ...
    tdb = TestDatabase()
    meta[['jid', 'ct', 'duration', 'req_cnt']] = meta.apply(fetchLatestJID(tdb, p__p), axis=1, result_type="expand")
    tdb.close()
    ...
    meta[['correct_cnt', 'cnt', 'dur_min', 'exe_t_ql']] = meta.apply(fetchTestDetail(tdb, p__p), axis=1, result_type="expand")
    tdb.close()
    # 画图
...
def plotlyChart(meta):
    expandData = meta[['max_con', 'throughput', 'dataset', 'db', 'ct', 'correct_cnt', 'dur_min']].explode('throughput').reset_index()
    ...
    fgl.update_traces(textfont_size=12, textposition="top center", texttemplate="%{text:.0f}")  # width=1.0,, cliponaxis=Falsetextangle=0,
