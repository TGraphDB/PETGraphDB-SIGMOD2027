...
{
    private final NavigableMap<TimePointInt, V> map = new TreeMap<>();
    ...
    public void set( TimePointInt start, TimePointInt end, V value ) {
        ...
        Entry<TimePointInt, V> curEndNext = map.floorEntry(end.next());
        V endNextVal = curEndNext == null ? null : curEndNext.getValue();
        map.subMap( start, true, end, true ).clear();
        map.put( start, value );
        map.put( end.next(), endNextVal );
    }
    ...
    public void insert(TimePointInt start, V value ){
        map.put(start, value);
    }
    ...
    public TimePointInt floorTime( TimePointInt time ) {
        return map.floorKey( time );
    }
