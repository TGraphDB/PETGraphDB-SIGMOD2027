...
public class PostgreSQLTimeIntervalClient extends PostgreSqlExecutorClient {
    ...
    protected List<String> createTables() {
        ...
        return Arrays.asList(
                "CREATE TABLE IF NOT EXISTS node(id BIGSERIAL PRIMARY KEY,"+content(schema.nodeStatic)+")",
                "CREATE TABLE IF NOT EXISTS rel (id BIGSERIAL PRIMARY KEY, r_from BIGINT, r_to BIGINT, "+content(schema.relStatic)+")",
                "CREATE TABLE IF NOT EXISTS node_tp(t tsrange, entity BIGINT "+(ntp.length()==0?"":","+ntp)+" )",
                "CREATE TABLE IF NOT EXISTS rel_tp (t tsrange, entity BIGINT "+(rtp.length()==0?"":","+rtp)+" )"
        );
    }
    ...
    public List<String> createIndexes() {
        return Arrays.asList(
                ...
                "CREATE UNIQUE INDEX IF NOT EXISTS n_tp_id_t on node_tp(entity, t)",
                "CREATE UNIQUE INDEX IF NOT EXISTS r_tp_id_t on rel_tp(entity, t)"
        );
    }
    ...
    private Req execute(SnapshotAggrMaxTx tx) {
        return conn -> {
            ...
            try (PreparedStatement stmt2 = conn.prepareStatement(
                    ...
                    "(SELECT entity, MAX(t) AS max_t FROM "+tableName+"_tp WHERE t<=? GROUP BY entity) AS tmp " +
                    "WHERE ts.entity=tmp.entity AND ss.id=tmp.entity AND ts.t>=tmp.max_t AND ts.t<=? GROUP BY u_sid")) {
    ...
    private Req execute(SnapshotAggrDurationTx tx) {
        ...
        return conn -> {
            ...
            try (PreparedStatement stmt0 = conn.prepareStatement("SELECT u_sid, t, " + tx.getP() + " FROM "+tableName+" AS ss, "+tableName+"_tp AS ts, "+
                    "(SELECT entity, MAX(t) AS max_t FROM "+tableName+"_tp WHERE t<=? GROUP BY entity) AS tmp " +
                    "WHERE ts.entity=tmp.entity AND ss.id=tmp.entity AND ts.t>=tmp.max_t AND ts.t<=? ORDER BY u_sid, t")) {
    ...
    private Req execute(EntityTemporalConditionTx tx) {
        return conn -> {
            ...
            try (PreparedStatement stmt0 = conn.prepareStatement("SELECT DISTINCT u_sid FROM "+tableName+" AS ss, " +tableName+"_tp AS ts, "+
                    "(SELECT entity, MAX(t) AS max_t FROM "+tableName+"_tp WHERE t<=? GROUP BY entity) AS tmp " +
                    "WHERE ss.id=ts.entity AND ss.id=tmp.entity AND ts."  + tx.getP() + " >= ? AND ts." + tx.getP() + " <= ? AND "+
        ...
        return conn -> {
            ...
            try (PreparedStatement stmt = conn.prepareStatement("SELECT id, u_sid FROM node");
                 PreparedStatement stmt0 = conn.prepareStatement("SELECT t, "+tx.getProp()+" AS prop FROM rel_tp AS ts, "+
                         "(SELECT entity, MAX(t) AS max_t FROM rel_tp WHERE entity=? AND t<=? GROUP BY entity) AS tmp " +
                         "WHERE ts.entity=tmp.entity AND tmp.max_t<=ts.t AND ts.t<=? ORDER BY t");
