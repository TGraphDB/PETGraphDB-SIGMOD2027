...
import org.neo4j.kernel.temporal.TemporalPropertyWriteOperation;
...
public class Operations implements Write, SchemaWrite
{
    ...
    private void validateType(Value value, ValueContentType type) {
        ValueContentType propertyType = ValueContentType.decode((Integer) value.asObjectCopy());
        if (!type.equals(propertyType)) {
            throw new TPSNHException("value type error: temporal property type " + propertyType + " but try to set " + type + " value!");
        }
    }

    private boolean nodeTemporalPropertyTypeExists(int propertyKey, ValueContentType newValueType) {
        nodeCursor.properties(propertyCursor, PropertySelection.selection(propertyKey));
        if (propertyCursor.next()) {
            validateType(propertyCursor.propertyValue(), newValueType);
            return true;
        }
        return false;
    }

    private boolean relationshipTemporalPropertyTypeExists(int propertyKey, ValueContentType newValueType) {
        relationshipCursor.properties(propertyCursor, PropertySelection.selection(propertyKey));
        if (propertyCursor.next()) {
            validateType(propertyCursor.propertyValue(), newValueType);
            return true;
    ...
    private void acquireExclusiveTemporalNodeLock(long node, long property, long start, long end) {
        if ( !ktx.hasTxStateWithChanges() || !ktx.txState().nodeIsAddedInThisTx( node ) )
        {
            ktx.lockClient().acquireTemporalExclusive( ktx.lockTracer(), ResourceTypes.NODE, node, property, start, end);
        }
    }

    private void acquireExclusiveTemporalRelationshipLock(long relationshipId, long property, long start, long end) {
        if ( !ktx.hasTxStateWithChanges() || !ktx.txState().relationshipIsAddedInThisTx( relationshipId ) )
        {
            ktx.lockClient().acquireTemporalExclusive( ktx.lockTracer(), ResourceTypes.RELATIONSHIP, relationshipId, property, start, end );
        }
    }
    ...
    @Override
    public void nodeSetTemporalProperty(TemporalPropertyWriteOperation operation) throws KernelException {
        acquireExclusiveTemporalNodeLock(operation.getEntityId(), operation.getProId(),
                operation.getInternalKey().getStartTime().getTime(), operation.getEnd().getTime());
        ktx.assertOpen();
        singleNode( operation.getEntityId() );

        ValueType valueType = operation.getInternalKey().getValueType();

        allStoreHolder.createNodeTemporalPropertyIfNotExists(operation.getProId(), valueType);
        if ( operation.getInternalKey().getValueType() != ValueType.INVALID )
        {
            ValueContentType type = valueType.toValueContentType();
            if (!nodeTemporalPropertyTypeExists(operation.getProId(), type)) {
                ktx.txState().nodeDoAddProperty(operation.getEntityId(), operation.getProId(), Values.of(type.getId()));
            }
            operation.setValueSlice( toSlice( operation.getValue() ) );
        }
        else
        {
            operation.setValueSlice( new Slice( 0 ) );
        }
        ktx.txState().nodeDoSetTemporalProperty(operation);
    }

    @Override
    public void relationshipSetTemporalProperty(TemporalPropertyWriteOperation operation) throws EntityNotFoundException {
        acquireExclusiveTemporalRelationshipLock(operation.getEntityId(), operation.getProId(),
                operation.getInternalKey().getStartTime().getTime(), operation.getEnd().getTime());
        ktx.assertOpen();
        singleRelationship(operation.getEntityId() );

        ValueType valueType = operation.getInternalKey().getValueType();

        allStoreHolder.createRelTemporalPropertyIfNotExists(operation.getProId(), valueType);
        if ( operation.getInternalKey().getValueType() != ValueType.INVALID )
        {
            ValueContentType type = valueType.toValueContentType();
            if (!relationshipTemporalPropertyTypeExists(operation.getProId(), type)) {
                ktx.txState().relationshipDoReplaceProperty(operation.getEntityId(), relationshipCursor.type(),
                        relationshipCursor.sourceNodeReference(), relationshipCursor.targetNodeReference(),
                        operation.getProId(), NO_VALUE, Values.of(type.getId()));
            }
            operation.setValueSlice( toSlice( operation.getValue() ) );
        }
        else
        ...
        ktx.txState().relationshipDoSetTemporalProperty(operation);
    }
}
