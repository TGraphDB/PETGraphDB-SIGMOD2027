...
import org.neo4j.graphdb.temporal.TGraphNotImplementedException;
...
import org.neo4j.kernel.temporal.TemporalPropertyReadOperation;
import org.neo4j.kernel.temporal.TemporalStorageProxy;
...
public class AllStoreHolder extends Read
{
    ...
    private Object temporalTxStatePointQuery(TemporalPropertyReadOperation query, MemTable txStateMemtable, ValueContentType valueType) throws ValueUnknownException {
        Slice value = txStateMemtable.get( new InternalKey( query.getProId(), query.getEntityId(), query.getStart(), ValueType.VALUE ) );
        if ( value == null )
        {
            return null;
        }
    ...
    private Object temporalTxStateAndStoreRangeQuery(TemporalPropertyReadOperation query, MemTable txStateMemtable, TemporalPropertyStore store, ValueContentType valueType) throws ValueUnknownException {
        TemporalStorageProxy proxy = databaseDependencies.resolveDependency(TemporalStorageProxy.class);
        return proxy.getRange(store, query, txStateMemtable);
    }

        private Object temporalStorePointQuery( TemporalPropertyReadOperation query, TemporalPropertyStore store, ValueContentType valueType )
    {
        TemporalStorageProxy proxy = databaseDependencies.resolveDependency(TemporalStorageProxy.class);
        Slice value = proxy.getPoint( store, query );
        if ( value != null )
        {
            return fromSlice( valueType, value );
        }
    ...
    private Object temporalStoreRangeQuery( TemporalPropertyReadOperation query, TemporalPropertyStore store, ValueContentType valueType )
    {
        TemporalStorageProxy proxy = databaseDependencies.resolveDependency(TemporalStorageProxy.class);
        return proxy.getRange(store, query, null);
    }

    private Object getTemporalProperty(TemporalPropertyReadOperation operation, ValueContentType type,
                                       MemTable txStateMemTable, TemporalPropertyStore store) {
        if (ktx.txState().hasDataChanges()) {
            try {
                if (operation.isPointQuery()) {
                    return temporalTxStatePointQuery(operation, txStateMemTable, type);
                } else if (operation.isRangeQuery()) {
                    return temporalTxStateAndStoreRangeQuery(operation, txStateMemTable, store, type);
                } else {
                    throw new UnsupportedOperationException(new TGraphNotImplementedException());
                }
            }
            catch (ValueUnknownException ignored) {}
        }
        if (operation.isPointQuery()) {
            return temporalStorePointQuery(operation, store, type);
        } else if (operation.isRangeQuery()) {
            return temporalStoreRangeQuery(operation, store, type);
        } else {
            throw new UnsupportedOperationException(new TGraphNotImplementedException());
        }
    }

    private TemporalStorageProxy getTemporalStorageProxy() {
        return databaseDependencies.resolveDependency(TemporalStorageProxy.class);
    }

    @Override
    public ValueContentType getNodeTemporalPropertyType(int propertyId) {
        return getTemporalStorageProxy().getNodeStore().getPropertyValueType(propertyId);
    }

    @Override
    public ValueContentType getRelationshipTemporalPropertyType(int propertyId) {
        return getTemporalStorageProxy().getRelStore().getPropertyValueType(propertyId);
    }

    /** 目前的时态存储中type不重要，重要的是为propertyId进行注册 */
    public void createNodeTemporalPropertyIfNotExists(int propertyId, ValueType type) {
        TemporalPropertyStore store = getTemporalStorageProxy().getNodeStore();
        if (store.getPropertyValueType(propertyId) != null) {
            return;
        }
        ValueContentType contentType = type.toValueContentType();
        Kernel kernel = databaseDependencies.resolveDependency(Kernel.class);
        try (KernelTransaction ktx = kernel.beginTransaction(KernelTransaction.Type.IMPLICIT, LoginContext.AUTH_DISABLED)) {
            ktx.tokenWrite().nodeCreateTemporalPropertyType(propertyId, contentType);
            ktx.commit();
        }
        catch (TransactionFailureException e) {
            throw new org.neo4j.graphdb.TransactionFailureException("error trying to create temporal property", e);
        }
    }

    /** 目前的时态存储中type不重要，重要的是为propertyId进行注册 */
    public void createRelTemporalPropertyIfNotExists(int propertyId, ValueType type) {
        TemporalPropertyStore store = getTemporalStorageProxy().getRelStore();
        if (store.getPropertyValueType(propertyId) != null) {
            return;
        }
        ValueContentType contentType = type.toValueContentType();
        Kernel kernel = databaseDependencies.resolveDependency(Kernel.class);
        try (KernelTransaction ktx = kernel.beginTransaction(KernelTransaction.Type.IMPLICIT, LoginContext.AUTH_DISABLED)) {
            ktx.tokenWrite().relationshipCreateTemporalPropertyType(propertyId, contentType);
            ktx.commit();
        }
        catch (TransactionFailureException e) {
            throw new org.neo4j.graphdb.TransactionFailureException("error trying to create temporal property", e);
    ...
    @Override
    public Object nodeGetTemporalProperty(TemporalPropertyReadOperation operation, ValueContentType type) {
        return getTemporalProperty(operation, type, ktx.txState().getNodeTemporalProperties(),
                getTemporalStorageProxy().getNodeStore());
    }

    @Override
    public Object relationshipGetTemporalProperty(TemporalPropertyReadOperation operation, ValueContentType type) {
        return getTemporalProperty(operation, type, ktx.txState().getRelationshipTemporalProperties(),
                getTemporalStorageProxy().getRelStore());
    }
}
