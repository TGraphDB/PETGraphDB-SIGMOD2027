...
public class ForsetiLockManager implements Locks
{
    ...
    /** When modifying temporalLockMaps, two things need to be ensured: no empty inner map can be re-created, which is
     *  guaranteed by ConcurrentHashMap; and an empty inner map which is just created and ready to put data cannot be
     *  cleared by clear thread here. This lock is used for this. When clear thread start working, it first acquires this
     *  writeLock, then access temporalLockMaps, and release it after clearing. When adding things to inner maps
     *  (possibly create one if no corresponding inner map exists), a readLock needs to be acquired first, and released
     *  after TemporalLock is added and the inner map is not null. When a transaction stops and releases locks, there is
     *  no need to lock this lock, because an obsolete inner map is not empty and thus will not be killed by clear thread.
     *  clear thread will only kill those maps which are empty when detected. This will only leave some empty maps not
     *  killed, and that is completely legitimate(this mechanism is just for preventing memory exhaustion, after all).
     */
    private final ReadWriteLock temporalLockMapsAccessLock = new ReentrantReadWriteLock(true);

    /**
     * To avoid generating too much empty inner maps and running out of memory, a thread to clear empty inner maps
     * periodically is introduced. No need to run too frequently.
     * !!Now this mechanism is temporally removed for stability, maybe it will be added back in the future.!!
     */
    ...
    public ForsetiLockManager( Config config, SystemNanoClock clock, ResourceType... resourceTypes )
    {
        ...
        this.temporalLockMaps = new ConcurrentMap[maxResourceId];

        for ( ResourceType type : resourceTypes )
        {
            this.lockMaps[type.typeId()] = new ConcurrentHashMap<>( 16, 0.6f, 512 );
            this.resourceTypes[type.typeId()] = type;
            this.temporalLockMaps[type.typeId()] = new ConcurrentHashMap<>(16, 0.6f, 512);
        }
        this.clock = clock;
        this.verboseDeadlocks = config.get( GraphDatabaseInternalSettings.lock_manager_verbose_deadlocks );
        /*this.scheduler = Executors.newScheduledThreadPool(1);
        // clear the map per day
        Thread task = new Thread(() -> {
            temporalLockMapsAccessLock.writeLock().lock();
            try {
                for (ConcurrentMap<Pair<Long, Long>, ConcurrentMap<Long, TemporalLock>> temporalLockMap : temporalLockMaps) {
                    if (temporalLockMap == null) continue;
                    temporalLockMap.entrySet().removeIf(entry -> entry.getValue().isEmpty());
                }
            }
            catch (Throwable e) {
                e.printStackTrace();
                ...
                temporalLockMapsAccessLock.writeLock().unlock();
            }
        });
        task.setDaemon(true);
        this.task = scheduler.scheduleAtFixedRate(task, 1, 1, TimeUnit.DAYS);*/
    }
    ...
    public Client newClient()
    {
        ...
        return new ForsetiClient( lockMaps, clock, verboseDeadlocks, clientIds.incrementAndGet(), temporalLockMaps, temporalLockMapsAccessLock );
    }
