...
public class ForsetiClient implements Locks.Client
{
    ...
    /** Key of the outer map means entity id + property id, key of an internal map means transaction id, value of internal map
     *  means the lock held by the transaction, value of outer map means the locks added on the specific property.<p/>
     *  the inner maps do not need synchronization because a TemporalLock object can only be added, modified or deleted by
     *  the corresponding transaction.*/
    private final ConcurrentMap<Pair<Long, Long>, ConcurrentMap<Long, TemporalLock>>[] temporalLockMaps;
    private final ReadWriteLock temporalLockMapsAccessLock;
    ...
    /**
     * The TemporalLock in this Ref is exactly the object in temporalLockMaps. This is an optimization to ensure
     * we don't need to coordinate if we grab the same temporal lock multiple times. It's also an index to record
     * how many locks have been created by this transaction, for unlocking.<p/>
     * Here is an example of the same TemporalLock t1 and t2 in <code>temporalLockMaps</code> and <code>localTemporalLockRef</code>:<p/>
     * <code>Pair p = Pair.of(entityId, propertyId)</code><p/>
     * <code>TemporalLock t1 = temporalLockMaps[rid].get(p).get(transactionId)</code><p/>
     * <code>TemporalLock t2 = localTemporalLockRef.get(rid).get(p)</code>
     */
    private final HashMap<Pair<Long, Long>, TemporalLock>[] localTemporalLockRef;
    ...
    private final Set<ForsetiClient> temporalWaitList = ConcurrentHashMap.newKeySet();
    ...
    private volatile boolean isWaitingTemporalLock = false;
    private volatile ConcurrentMap<Long, TemporalLock> pointLockMapOfTemporalWaitingForPoint;
    private volatile boolean isWaitingSharedTemporalLock;
    private volatile long waitingStart, waitingEnd;
    ...
    public ForsetiClient(ConcurrentMap<Long,ForsetiLockManager.Lock>[] lockMaps, SystemNanoClock clock, boolean verboseDeadlocks, long clientId,
                         ConcurrentMap<Pair<Long, Long>, ConcurrentMap<Long, TemporalLock>>[] temporalLockMaps, ReadWriteLock temporalLockMapsAccessLock)
    {
        this.lockMaps = lockMaps;
        this.temporalLockMaps  = temporalLockMaps;
        this.temporalLockMapsAccessLock = temporalLockMapsAccessLock;
        ...
        this.localTemporalLockRef = new HashMap[temporalLockMaps.length];
        this.clock = clock;
    ...
    private TemporalLock getInTemporalLockRef(ResourceType resourceType, long entityId, long propertyId) {
        HashMap<Pair<Long, Long>, TemporalLock> temp = localTemporalLockRef[resourceType.typeId()];
        if (temp == null) {
            temp = new HashMap<>();
            localTemporalLockRef[resourceType.typeId()] = temp;
        }
        return temp.get(Pair.pair(entityId, propertyId));
    }

    /** this method gets the corresponding TemporalLock in localTemporalRef(which is in temporalLockMaps, too),
     *  if no lock is found, then create a lock and put it into temporalLockMaps and localTemporalRef,
     *  then return the created lock. */
    private TemporalLock getOrCreateTemporalLock(ResourceType resourceType, long entityId, long propertyId) {
        TemporalLock localLock = getInTemporalLockRef(resourceType, entityId, propertyId);
        if (localLock != null) {
            return localLock;
        }
        // transaction has not locked this entity property yet, so create a lock and put it into the global and local maps
        localLock = new TemporalLock(this);
        Pair<Long, Long> key = Pair.pair(entityId, propertyId);
        temporalLockMapsAccessLock.readLock().lock();
        try {
            ConcurrentMap<Pair<Long, Long>, ConcurrentMap<Long, TemporalLock>> temporalLockMap = temporalLockMaps[resourceType.typeId()];
            ConcurrentMap<Long, TemporalLock> temp = temporalLockMap.putIfAbsent(key, new ConcurrentHashMap<>());
            if (temp == null) {
                // successfully put an empty map into the outer map, so get the map we just put.
                // no code exchanges the inner map, and the code deleting the inner map must acquire a writeLock first,
                // so the inner map gotten here must be the empty map we just put in
                temp = temporalLockMap.get(key);
            }
            else {
                // else, temp is exactly what we want
            }
            // put the lock into global map to ensure the inner map won't be cleared by accident
            temp.put(transactionId, localLock);
        }
        finally {
            temporalLockMapsAccessLock.readLock().unlock();
        }
        // put into local map
        localTemporalLockRef[resourceType.typeId()].put(key, localLock);
        return localLock;
    }
    ...
    private void checkTemporalResourceType(ResourceType resourceType) {
        if (resourceType != ResourceTypes.RELATIONSHIP && resourceType != ResourceTypes.NODE) {
            throw new RuntimeException("unsupported temporal lock!");
        }
    }

    @Override
    public void acquireTemporalShared(LockTracer tracer, ResourceType resourceType, long entityId, long propertyId, long start, long end) {
        checkTemporalResourceType(resourceType);
        acquireShared(tracer, resourceType, entityId);
        stateHolder.incrementActiveClients(this);
        try {
            TemporalLock temporalLock = getOrCreateTemporalLock(resourceType, entityId, propertyId);
            // it's safe not to do synchronization here, because this temporal belongs to this transaction and no other
            // transaction can update it
            if (temporalLock.sharedLocked(start, end)) {
                // this interval has been locked by this transaction, so just return
                return;
            }
            // now try to grab the shared lock
            ConcurrentMap<Long, TemporalLock> pointLockMap = temporalLockMaps[resourceType.typeId()].get(Pair.pair(entityId, propertyId));
            int tries = 0;
            while (true) {
                // We need to synchronize pointLockMap to ensure wo get the exact intersection situation and lock it.
                // To simplify code, we just use synchronized to modify pointLockMap here, read write lock may be introduced
                // further if needed.
                boolean success;
                synchronized (pointLockMap) {
                    boolean canLock = true;
                    for (Map.Entry<Long, TemporalLock> entry : pointLockMap.entrySet()) {
                        if (entry.getKey() == transactionId) continue;
                        if (entry.getValue().sharedConflict(start, end)) {
                            canLock = false;
                            break;
                        }
                    }
                    if (!canLock) {
                        ...
                        temporalLock.lockShared(start, end);
                        success = true;
                    }
                }
                if (success) {
                ...
                temporalWaitFor(pointLockMap, true, start, end, tries++);
            }
        }
        finally {
            clearTemporalWaitList();
            stateHolder.decrementActiveClients();
            isWaitingTemporalLock = false;
        }
    }

    @Override
    public void acquireTemporalExclusive(LockTracer tracer, ResourceType resourceType, long entityId, long propertyId, long start, long end) {
        checkTemporalResourceType(resourceType);
        acquireShared(tracer, resourceType, entityId);
        stateHolder.incrementActiveClients(this);
        try {
            ...
            // it's safe not to do synchronization here, because this temporal belongs to this transaction and no other
            // transaction can update it
            if (temporalLock.exclusiveLocked(start, end)) {
                // this interval has been locked by this transaction, so just return
                return;
            }
            // now try to grab the exclusive lock
            ConcurrentMap<Long, TemporalLock> pointLockMap = temporalLockMaps[resourceType.typeId()].get(Pair.pair(entityId, propertyId));
            boolean sharedAcquired = temporalLock.sharedLocked(start, end);
            boolean upgradeSuccess = temporalLock.upgradeLocked(start, end);
            int tries = 0;
            while (true) {
                boolean success;
                synchronized (pointLockMap) {
                    // operations here are atomic, so if a shared lock is acquired or can be acquired here, it can be
                    // upgraded immediately.
                    // first try to acquire an exclusive lock
                    boolean canLockExclusive = true;
                    for (Map.Entry<Long, TemporalLock> entry : pointLockMap.entrySet()) {
                        if (entry.getKey() == transactionId) continue;
                        if (entry.getValue().exclusiveConflict(start, end, false)) {
                            canLockExclusive = false;
                            break;
                        }
                    }
                    if (canLockExclusive) {
                        temporalLock.lockExclusive(start, end);
                        success = true;
                    }
                    else {
                        success = false;
                        // try to acquire a shared lock then upgrade it
                        if (!upgradeSuccess) {
                            if (!sharedAcquired) {
                                boolean canLockShared = true;
                                for (Map.Entry<Long, TemporalLock> entry : pointLockMap.entrySet()) {
                                    if (entry.getKey() == transactionId) continue;
                                    if (entry.getValue().sharedConflict(start, end)) {
                                        canLockShared = false;
                            ...
                            if (sharedAcquired) {
                                temporalLock.upgradeShared(start, end);
                                upgradeSuccess = true;
                            }
                        }
                    }
                ...
                temporalWaitFor(pointLockMap, false, start, end, tries++);
            }
        }
        finally {
            clearTemporalWaitList();
    ...
    private void temporalWaitFor(ConcurrentMap<Long, TemporalLock> pointLockMap, boolean acquiringShared, long start, long end, int tries) {
        pointLockMapOfTemporalWaitingForPoint = pointLockMap;
        isWaitingSharedTemporalLock = acquiringShared;
        waitingStart = start;
        waitingEnd = end;
        // set this to true last to ensure that when this is true, the values set above is valid
        isWaitingTemporalLock = true;
        clearTemporalWaitList();
        ForsetiClient deadlockWith = getMapWaitListAndDetectDeadLock();
        // we use the normal shouldAbort because a shared lock is always added before adding a temporal lock, so active
        // lock count can indicate how many temporal locks have been added.
        if (deadlockWith != null && shouldAbort(deadlockWith)) {
            if (tries > 100 && isTemporalDeadlockReal()) {
                // After checking several times, this really does look like a real deadlock.
                throw new DeadlockDetectedException("there is likely a deadlock occurred, and " + this + "is now terminated");
            }
            Thread.yield();
        }
    }

    private boolean isTemporalDeadlockReal() {
        if ( isTemporalDeadlockRealInternal() )
        {
            parkNanos( MILLISECONDS.toNanos( 10 ) );
            return isTemporalDeadlockRealInternal();
        }
        return false;
    }

    private boolean isTemporalDeadlockRealInternal() {
        // use BFS
        Set<ForsetiClient> collectedClients = new HashSet<>(), nextClients = new HashSet<>();
        LinkedList<ForsetiClient> unvisitedClients = new LinkedList<>();
        unvisitedClients.add(this);
        while (!unvisitedClients.isEmpty()) {
            ForsetiClient client = unvisitedClients.pollFirst();
            collectDirectTemporalWaitingFor(client, nextClients);
            if (nextClients.contains(this)) {
                return true;
            }
            for (ForsetiClient c : nextClients) {
                if (!collectedClients.contains(c)) {
                    collectedClients.add(c);
                    unvisitedClients.add(c);
                }
            }
            nextClients.clear();
        }
    ...
    private ForsetiClient getMapWaitListAndDetectDeadLock() {
        ForsetiClient deadlockWith = null;
        Set<ForsetiClient> waitingForList = new HashSet<>();
        collectDirectTemporalWaitingFor(this, waitingForList);
        for (ForsetiClient waitingForClient : waitingForList) {
            waitingForClient.copyTemporalWaitListTo(temporalWaitList);
            if (waitingForClient.isWaitingForTemporally(this)) {
                deadlockWith = waitingForClient;
            }
        }
        return deadlockWith;
    }

    // client C is waiting on pointLockMap for a temporal shared/exclusive lock(acquiringShared) at time range [start, end],
    // collect the clients that block client C on this point and put it into receiver
    private static void collectDirectTemporalWaitingFor(ForsetiClient client, Collection<ForsetiClient> receiver) {
        if (!client.isWaitingTemporalLock) return;
        ConcurrentMap<Long, TemporalLock> pointLockMap = client.pointLockMapOfTemporalWaitingForPoint;
        boolean acquiringShared = client.isWaitingSharedTemporalLock;
        long start = client.waitingStart, end = client.waitingEnd;
        for (Map.Entry<Long, TemporalLock> entry : pointLockMap.entrySet()) {
            if (entry.getKey() == client.transactionId) continue;
            boolean isWaitingFor;
            if (acquiringShared) {
                isWaitingFor = entry.getValue().sharedConflict(start, end);
            }
            else {
                isWaitingFor = entry.getValue().exclusiveConflict(start, end, false);
            }
            if (isWaitingFor) {
                receiver.add(entry.getValue().getOwner());
            }
        }
    }

    boolean isWaitingForTemporally(ForsetiClient client) {
        return this.transactionId != client.transactionId && this.temporalWaitList.contains(client);
    }

    void copyTemporalWaitListTo(Set<ForsetiClient> receiver) {
        receiver.add(this);
        receiver.addAll(temporalWaitList);
    }
    ...
    private void releaseAllClientTemporalLocks() {
        for (int i = 0; i < localTemporalLockRef.length; i++) {
            HashMap<Pair<Long, Long>, TemporalLock> tempMap = localTemporalLockRef[i];
            ConcurrentMap<Pair<Long, Long>, ConcurrentMap<Long, TemporalLock>> temporalLockMap = temporalLockMaps[i];
            if (tempMap == null) continue;
            for (Map.Entry<Pair<Long, Long>, TemporalLock> entry : tempMap.entrySet()) {
                ConcurrentMap<Long, TemporalLock> globalMapOfOnePoint = temporalLockMap.get(entry.getKey());
                if (globalMapOfOnePoint == null) throw new RuntimeException("temporal lock concurrency error!");
                globalMapOfOnePoint.remove(transactionId);
            }
            tempMap.clear();
        }
    }
    ...
    private void clearTemporalWaitList() {
        temporalWaitList.clear();
    }
