...
public class NodeEntity implements Node, RelationshipFactory<Relationship>
{
    ...
    public void delete()
    {
        ...
        keys.forEach(key -> {
            ...
            ValueContentType type = transaction.schemaRead().getNodeTemporalPropertyType(propertyKeyId);
            if (type == null) {
                return;
            }
            ...
            try
            {
                TemporalPropertyWriteOperation operation = new TemporalPropertyWriteOperation(nodeId, propertyKeyId, new TimePoint(0), TimePoint.NOW, null);
                transaction.dataWrite().nodeSetTemporalProperty(operation);
            }
    ...
    public void setTemporalProperty(String key, TimePoint start, TimePoint end, Object value) {
        ...
        try
        {
            TemporalPropertyWriteOperation operation = new TemporalPropertyWriteOperation(nodeId, propertyKeyId, start, end, value);
            transaction.dataWrite().nodeSetTemporalProperty(operation);
        }
        catch ( ConstraintValidationException e )
        {
    ...
    public Object getTemporalProperty(String key, TimePoint time) {
        ...
        return transaction.dataRead().nodeGetTemporalProperty( query, ValueContentType.decode((Integer) o));
    }
    ...
    public void acquireTemporalSharedLock(String key, TimePoint start, TimePoint end) {
        ...
        transaction.locks().acquireNodeTemporalSharedLock(nodeId, propertyKey, start.getTime(), end.getTime());
    }
    ...
    public void acquireTemporalExclusiveLock(String key, TimePoint start, TimePoint end) {
        ...
        transaction.locks().acquireNodeTemporalExclusiveLock(nodeId, propertyKey, start.getTime(), end.getTime());
    }
}
