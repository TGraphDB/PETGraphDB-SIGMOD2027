...
public class RelationshipEntity implements Relationship, RelationshipVisitor<RuntimeException>
{
    ...
    public void delete()
    {
        ...
        keys.forEach(key -> {
            ...
            ValueContentType type = transaction.schemaRead().getRelationshipTemporalPropertyType(propertyKeyId);
            if (type == null) {
                return;
            }
            ...
            try
            {
                TemporalPropertyWriteOperation operation = new TemporalPropertyWriteOperation(id, propertyKeyId, new TimePoint(0), TimePoint.NOW, null);
                transaction.dataWrite().relationshipSetTemporalProperty(operation);
            }
    ...
    public void setTemporalProperty(String key, TimePoint start, TimePoint end, Object value) {
        ...
        try
        {
            TemporalPropertyWriteOperation operation = new TemporalPropertyWriteOperation(id, propertyKeyId, start, end, value);
            transaction.dataWrite().relationshipSetTemporalProperty(operation);
        }
        catch ( IllegalArgumentException e )
        {
    ...
    public Object getTemporalProperty(String key, TimePoint time) {
        ...
        Object o = properties.propertyValue().asObjectCopy();
        TemporalPropertyReadOperation query = new TemporalPropertyReadOperation( this.getId(), propertyKey, time );
        return transaction.dataRead().relationshipGetTemporalProperty( query, ValueContentType.decode((Integer) o));
    }
    ...
    public Object getTemporalProperty(String key, TimePoint startTime, TimePoint endTime, TemporalRangeQuery callBack) {
        ...
        Object o = properties.propertyValue().asObjectCopy();
        TemporalPropertyReadOperation query = new TemporalPropertyReadOperation( this.getId(), propertyKey, startTime, endTime, callBack );
        return transaction.dataRead().relationshipGetTemporalProperty( query, ValueContentType.decode((Integer) o));
    }

    @Override
    public void acquireTemporalSharedLock(String key, TimePoint start, TimePoint end) {
        if ( null == key )
        {
            throw new IllegalArgumentException( "(null) property key is not allowed" );
        }
        ...
        transaction.locks().acquireRelationshipTemporalSharedLock(id, propertyKey, start.getTime(), end.getTime());
    }

    @Override
    public void acquireTemporalExclusiveLock(String key, TimePoint start, TimePoint end) {
        if ( null == key )
        {
            throw new IllegalArgumentException( "(null) property key is not allowed" );
        }
        ...
        transaction.locks().acquireRelationshipTemporalExclusiveLock(id, propertyKey, start.getTime(), end.getTime());
    }
}
