...
public class InternalTransactionCommitProcess implements TransactionCommitProcess
{
    ...
    public static class Debug {
        ...
        public static boolean mockLog = false;
        public static boolean mockStore = false;
    }
    ...
    {
        long lastTxId = Debug.mockLog ? 0 : appendToLog( batch, commitEvent );
        if (Debug.debugging) {
            if (Debug.crash) {
                System.exit(0);
            }
        ...
        try
        {
            if (!Debug.mockStore) applyToStore( batch, commitEvent, mode );
            return lastTxId;
        }
        finally
        {
