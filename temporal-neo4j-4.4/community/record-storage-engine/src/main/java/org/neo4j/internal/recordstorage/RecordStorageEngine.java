...
public class RecordStorageEngine implements StorageEngine, Lifecycle
{
    ...
    private TemporalPropertyStore nodeStore = null, relStore = null;
    ...
    public RecordStorageEngine( RecordDatabaseLayout databaseLayout,
                                ...
                                CommandLockVerification.Factory commandLockVerificationFactory,
                                LockVerificationMonitor.Factory lockVerificationFactory,
                                TemporalPropertyStore nodeStore,
                                TemporalPropertyStore relStore
    ) {
        this(databaseLayout, config, pageCache, fs, internalLogProvider, userLogProvider, tokenHolders, schemaState,
                constraintSemantics, indexConfigCompleter, lockService, databaseHealth, idGeneratorFactory, idController,
                recoveryCleanupWorkCollector, cacheTracer, createStoreIfNotExists, otherMemoryTracker, readOnlyChecker,
                commandLockVerificationFactory, lockVerificationFactory);
    ...
    private TransactionApplierFactoryChain buildApplierFacadeChain( TransactionApplicationMode mode )
    {
        ...
        if (mode != REVERSE_RECOVERY) {
            appliers.add(new TemporalPropertyStoreApplierFactory(nodeStore, relStore));
        }
