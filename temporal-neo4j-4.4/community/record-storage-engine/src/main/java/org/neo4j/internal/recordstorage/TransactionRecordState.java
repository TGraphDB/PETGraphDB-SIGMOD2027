...
public class TransactionRecordState implements RecordState
{
    ...
    public void visitNodeTemporalPropertyCreation(HashMap<Integer, ValueContentType> changes) {
        this.nodeTemporalPropertyCreation = changes;
    }

    public void visitRelationshipTemporalPropertyCreation(HashMap<Integer, ValueContentType> changes) {
        this.relationshipTemporalPropertyCreation = changes;
    }

    private HashMap<Integer, ValueContentType> nodeTemporalPropertyCreation, relationshipTemporalPropertyCreation;
    ...
    public void extractCommands( Collection<StorageCommand> commands, MemoryTracker memoryTracker ) throws TransactionFailureException
    {
        ...
        {
            PeekingIterator<Map.Entry<TimeIntervalKey, Slice>> nodeTpIter = nodeTemporalPropertyChanges.intervalEntryIterator();
            while ( nodeTpIter.hasNext() )
            {
                Map.Entry<TimeIntervalKey,Slice> entry = nodeTpIter.next();
        ...
        {
            PeekingIterator<Map.Entry<TimeIntervalKey,Slice>> relTpIter = relationshipTemporalPropertyChanges.intervalEntryIterator();
            while ( relTpIter.hasNext() )
            {
                Map.Entry<TimeIntervalKey,Slice> entry = relTpIter.next();
        ...
        if (nodeTemporalPropertyCreation != null) {
            for (Map.Entry<Integer, ValueContentType> entry : nodeTemporalPropertyCreation.entrySet()) {
        ...
        if (relationshipTemporalPropertyCreation != null) {
            for (Map.Entry<Integer, ValueContentType> entry : relationshipTemporalPropertyCreation.entrySet()) {
